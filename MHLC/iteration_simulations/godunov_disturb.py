...
def mindisturb(disturbrec,begin_t,delta_t):
    ...
    dlc=[]
    dmlc=[]
    # rec[1] is the time object in the list we created
    # it change_to_sec
    # disturbrec is just the list
    for rec in disturbrec:
        # so rec[1] should be the value after change_to_second
        tt = rec[1]
    ...
    for rec in disturbrec:
        tt = rec[1]
        n = int((tt - change_to_sec(begin_t)) / delta_t)
        if rec[3]==1: # THIS IS THE TYPE for DLC
            dlc.append(n)
        if rec[3]==0: # type for MLC
            dmlc.append(n)
