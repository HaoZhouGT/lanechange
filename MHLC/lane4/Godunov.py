...
def F(Q, u, w, kj, ku, kd, cap, last_cell = False):
    supply = lambda Q, w, kj, k: min(Q, w*(kj-k))
    demand = lambda Q, u, k: min(Q, k*u)
    d_u = demand(Q, u, ku)
    if not last_cell:
        s_d = supply(Q, w, kj, kd)
    else:
        s_d = supply(cap, w, kj, kd)
    return min(d_u, s_d)
...
def Godunov(u, w, kj, delta_t, from_x, to_x,
    ...
    sol_v.append(k0_v)
    sol.append(k0)
    # initialize a list, append a list every time
    # we iterate by time, update the density of each road cell
    for i in range(0, iT-1):
        k = [upstream_dens[i+1]]
        k_v = [den_to_v(upstream_dens[i+1], u,w,kj)]
        for j in range(1, iL-1):
            if j != iL - 2:
                s = k0[j] + 1/u * (F(Q, u, w, kj, k0[j-1], k0[j], downstream_cap[i]) -
                                   F(Q, u, w, kj, k0[j], k0[j+1], downstream_cap[i])
                                   + alpha*inflow[j, i] - beta*outflow[j,i])
            else: # for the last cell, use the downstream capacity
                inflo = F(Q, u, w, kj, k0[j-1], k0[j], downstream_cap[i])
                ouflo = F(Q, u, w, kj, k0[j], k0[j+1], downstream_cap[i], True)
    ...
    # plt.figure(7)
    # plt.imshow(np.transpose(z)[::-1], aspect = asp, extent=ext)
    # plt.colorbar()
    # plt.xlabel('Time (seconds)')
    # plt.ylabel('Location(ft)')
    # plt.title('the simulated density of cells')
    # plt.show()

    plt.figure()
    plt.imshow(np.transpose(z_v)[::-1], aspect = asp, extent=ext, cmap = 'jet_r')
    plt.colorbar()
    plt.xlabel('Time (seconds)')
    plt.ylabel('Location(ft)')
    plt.title('the simulated speed of cells')
    plt.show()
    return z_v
